mutable struct CtrDrbg  <: AbstractRNG
    ...
    function CtrDrbg()
        ...
        Base.@threadcall((:mbedtls_ctr_drbg_init, libmbedcrypto), Cvoid, (Ptr{Cvoid},), ctx.data)
        finalizer(ctx->begin
...
function seed!(rng::CtrDrbg, entropy, pdata)
    ...
    @err_check Base.@threadcall((:mbedtls_ctr_drbg_seed, libmbedcrypto), Cint,
...
function Random.rand!(rng::CtrDrbg, buf::Array)
    @err_check Base.@threadcall((:mbedtls_ctr_drbg_random, libmbedcrypto), Cint,

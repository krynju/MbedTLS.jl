mutable struct Entropy
    ...
    function Entropy()
        ...
        ctx.sources = Any[]
        Base.@threadcall((:mbedtls_entropy_init, libmbedcrypto), Cvoid, (Ptr{Cvoid},), ctx.data)
        finalizer(ctx->begin
...
function add_source!(ctx::Entropy, f_source::Ptr, f, threshold, strong)
    ret = Base.@threadcall((:mbedtls_entropy_add_source, libmbedcrypto), Cint,
...
function gather(ctx::Entropy)
    @err_check Base.@threadcall((:mbedtls_entropy_gather, libmbedcrypto), Cint,
      (Ptr{Cvoid},), ctx.data)
end

function update_manual(ctx::Entropy, data::Vector{UInt8})
    @err_check Base.@threadcall((:mbedtls_entropy_update_manual, libmbedcrypto), Cint,

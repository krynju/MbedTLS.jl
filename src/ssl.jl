...
mutable struct SSLConfig
    ...
    function SSLConfig()
        ...
        Base.@threadcall((:mbedtls_ssl_config_init, libmbedtls), Cvoid, (Ptr{Cvoid},), conf.data)
        finalizer(x->begin
            data = x.data
            @async begin
...
mutable struct SSLContext <: IO
    ...
    function SSLContext()
        ...
        Base.@threadcall((:mbedtls_ssl_init, libmbedtls), Cvoid, (Ptr{Cvoid},), ctx.data)
        finalizer(x->begin
            data = x.data
            @async begin
...
function get_peer_cert(ctx::SSLContext)
    data = Base.@threadcall((:mbedtls_ssl_get_peer_cert, libmbedtls), Ptr{Cvoid}, (Ptr{Cvoid},), ctx.data)
    return CRT(data)
end

function get_version(ctx::SSLContext)
    if isdefined(ctx, :config)
        data = Base.@threadcall((:mbedtls_ssl_get_version, libmbedtls), Ptr{UInt8}, (Ptr{Cvoid},), ctx.data)
...
function get_ciphersuite(ctx::SSLContext)
    data = Base.@threadcall((:mbedtls_ssl_get_ciphersuite, libmbedtls), Ptr{UInt8}, (Ptr{Cvoid},), ctx.data)
    return unsafe_string(data)
end
...
function ssl_get_bytes_avail(ctx::SSLContext)::Int
    @lockdata ctx begin
        return Base.@threadcall((:mbedtls_ssl_get_bytes_avail, libmbedtls),
...
function ssl_check_pending(ctx::SSLContext)::Bool
    @lockdata ctx begin
    return Base.@threadcall((:mbedtls_ssl_check_pending, libmbedtls),
...
function ssl_set_bio(ctx::SSLContext, bio, f_send::Ptr{Cvoid}, f_recv::Ptr{Cvoid})
    @lockdata ctx begin
        Base.@threadcall((:mbedtls_ssl_set_bio, libmbedtls), Cvoid,
...
function ssl_read(ctx::SSLContext, ptr, n)::Int
    @lockdata ctx begin
        return Base.@threadcall((:mbedtls_ssl_read, libmbedtls), Cint,
...
function ssl_write(ctx::SSLContext, ptr, n)::Int
    @lockdata ctx begin
        return Base.@threadcall((:mbedtls_ssl_write, libmbedtls), Cint,
                     (Ptr{Cvoid}, Ptr{Cvoid}, Csize_t),
                     ctx.data, ptr, n)
    end
end
...
function ssl_close_notify(ctx::SSLContext)
    @lockdata ctx begin
        return Base.@threadcall((:mbedtls_ssl_close_notify, libmbedtls),
...
function ssl_session_reset(ctx::SSLContext)
    @lockdata ctx begin
        return Base.@threadcall((:mbedtls_ssl_session_reset, libmbedtls),
                     Cint, (Ptr{Cvoid},), ctx.data)
    end
end

function hostname!(ctx::SSLContext, hostname)
    @err_check Base.@threadcall((:mbedtls_ssl_set_hostname, libmbedtls), Cint,

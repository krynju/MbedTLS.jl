...
mutable struct MD{IsHMAC} <: IO
    ...
    function MD{IsHMAC}() where IsHMAC
        ...
        Base.@threadcall((:mbedtls_md_init, libmbedcrypto), Cvoid, (Ptr{Cvoid},), ctx.data)
        finalizer(ctx->begin
...
function MDInfo(kind::MDKind)
    ret = Base.@threadcall((:mbedtls_md_info_from_type, libmbedcrypto), Ptr{Cvoid},
        (Cint,), Int(kind))
    if ret == C_NULL
...
function MDInfo(kind::AbstractString)
    ret = Base.@threadcall((:mbedtls_md_info_from_string, libmbedcrypto), Ptr{Cvoid},
        (Cstring,), String(kind))
    MDInfo(ret)
end

function get_name(info::MDInfo)
    ret = Base.@threadcall((:mbedtls_md_get_name, libmbedcrypto), Ptr{UInt8},
        (Ptr{Cvoid},), info.data)
    unsafe_string(ret)
end
...
function MD(kind::MDKind)
    ...
    @err_check Base.@threadcall((:mbedtls_md_setup, libmbedcrypto), Cint,
        (Ptr{Cvoid}, Ptr{Cvoid}, Cint),
        ctx.data, ctx.info.data, 0)
    @err_check Base.@threadcall((:mbedtls_md_starts, libmbedcrypto), Cint,
        (Ptr{Cvoid},), ctx.data)
    ctx
end
...
function MD(kind::MDKind, key)
    ...
    @err_check Base.@threadcall((:mbedtls_md_hmac_starts, libmbedcrypto), Cint,
...
function Base.copy(md::MD)
    new_md = MD()
    @err_check Base.@threadcall((:mbedtls_md_clone, libmbedcrypto), Cint,
        (Ptr{Cvoid}, Ptr{Cvoid}),
        new_md.data, md.data)
    new_md.info = md.info
    new_md
...
function get_size(info::MDInfo)
    ret = Base.@threadcall((:mbedtls_md_get_size, libmbedcrypto), Cuchar,
        (Ptr{Cvoid},), info.data)
    Int(ret)
end
...
function _write(ctx::MD{false}, buf, size)
    @err_check Base.@threadcall((:mbedtls_md_update, libmbedcrypto), Cint,
        (Ptr{Cvoid}, Ptr{Cvoid}, Csize_t),
        ctx.data, buf, size)
end

function _write(ctx::MD{true}, buf, size)
    @err_check Base.@threadcall((:mbedtls_md_hmac_update, libmbedcrypto), Cint,
        (Ptr{Cvoid}, Ptr{Cvoid}, Csize_t),
        ctx.data, buf, size)
end
...
function finish!(ctx::MD{false}, buf)
    @err_check Base.@threadcall((:mbedtls_md_finish, libmbedcrypto), Cint,
        (Ptr{Cvoid}, Ptr{Cvoid}),
        ctx.data, pointer(buf))
end

function finish!(ctx::MD{true}, buf)
    @err_check Base.@threadcall((:mbedtls_md_hmac_finish, libmbedcrypto), Cint,
        (Ptr{Cvoid}, Ptr{Cvoid}),
        ctx.data, pointer(buf))
end
...
function reset!(ctx::MD{true})
    @err_check Base.@threadcall((:mbedtls_md_hmac_reset, libmbedcrypto), Cint,
        (Ptr{Cvoid},),
        ctx.data)
end

function digest!(kind::MDKind, msg, buf)
    @err_check Base.@threadcall((:mbedtls_md, libmbedcrypto), Cint,
...
function digest!(kind::MDKind, msg, key, buf)
    @err_check Base.@threadcall((:mbedtls_md_hmac, libmbedcrypto), Cint,
